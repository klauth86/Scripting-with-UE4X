...
AWarriorWithAbilities::AWarriorWithAbilities()
{
	...
	GameplayTasksComponent =
		CreateDefaultSubobject<UGameplayTasksComponent>
		("UGameplayTasksComponent");
}

// Called when the game starts or when spawned
void AWarriorWithAbilities::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AWarriorWithAbilities::Tick(float DeltaTime)
{
...
// Called to bind functionality to input
void AWarriorWithAbilities::SetupPlayerInputComponent(UInputComponent* Input) {
	...
	for (const FGameplayAbilityBindInfo& BindInfo : gameplayAbilitySet->Abilities) {
		...
		FGameplayAbilityInputBinds inputBinds(
			...
			FS("ConfirmTargetting_%s_%s", *GetName(), *BindInfo.GameplayAbilityClass->GetName()),
...
void AWarriorWithAbilities::PostInitializeComponents() {
...
void AWarriorWithAbilities::TestGameplayEffect() {
	// Construct & retrieve UProperty to affect
	UGameplayEffect* RecoverHP =
		ConstructGameplayEffect("RecoverHP");
	...
	AddModifier(RecoverHP, hpProperty, EGameplayModOp::Additive,
		FScalableFloat(50.f));
