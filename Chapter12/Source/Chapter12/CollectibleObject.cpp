...
void ACollectibleObject::OnBeginOverlap(
	...
	if (Role < ROLE_Authority) {
		...
		if (Role == ROLE_Authority) {
			GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, "OnBeginOverlap SERVER!!!");
		}
		else {
			GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, "OnBeginOverlap CLIENT!");
		}
...
void ACollectibleObject::UpdateScore_Implementation(int32 Amount) {
	if (Role == ROLE_Authority) {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, "UpdateScore_Implementation SERVER!!!");
	}
	else {
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, "UpdateScore_Implementation CLIENT!");
	}
